import{ICONS}from'../../../core/theme/icons.js';import{i18nService}from'../../../core/i18n/i18n.service.js';export class TrainingAddTrigger extends HTMLElement{static get observedAttributes(){return['label'];}
constructor(){super();}
async connectedCallback(){this.i18n=await i18nService.loadPage('training_planner/add-trigger');this.render();}
attributeChangedCallback(name,oldValue,newValue){if(oldValue!==newValue)this.render();}
render(){const label=this.getAttribute('label')||this.i18n.t('lbl_add_exercise');const iconSvg=ICONS.PLUS.replace('<svg','<svg viewBox="0 0 16 16"').replace('width="16"','width="100%"').replace('height="16"','height="100%"');this.innerHTML=`
        <style>
