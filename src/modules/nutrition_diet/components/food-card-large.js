import'../../nutrition_core/components/macro-badge.js';import{i18nService}from'../../../core/i18n/i18n.service.js';export class AppFoodCardLarge extends HTMLElement{static get observedAttributes(){return['label','k','p','c','f','grams'];}
constructor(){super();this.attachShadow({mode:'open'});this.dict=null;}
async connectedCallback(){this.dict=await i18nService.loadPage('nutrition_diet/food-card-large');this.render();this.addEventListener('click',()=>{this.dispatchEvent(new CustomEvent('edit-food',{bubbles:true,composed:true}));});}
attributeChangedCallback(name,oldValue,newValue){if(oldValue!==newValue&&this.dict)this.render();}
render(){const label=this.getAttribute('label')||this.dict.t('card_large_default_label');const k=Math.round(this.getAttribute('k')||0);const p=this.getAttribute('p')||'0';const c=this.getAttribute('c')||'0';const f=this.getAttribute('f')||'0';const rawGrams=this.getAttribute('grams')||'0';const displayGrams=/[a-z]/i.test(rawGrams)?rawGrams:`${rawGrams}G`;this.shadowRoot.innerHTML=`
        <style>
