import{ICONS}from'../../../core/theme/icons.js';import{i18nService}from'../../../core/i18n/i18n.service.js';export class TrainingRoutineCard extends HTMLElement{static get observedAttributes(){return['title','count','last-performed'];}
constructor(){super();this.attachShadow({mode:'open'});}
async connectedCallback(){this.i18n=await i18nService.loadPage('training_planner/routine-card');this.render();this._attachListeners();}
attributeChangedCallback(name,oldValue,newValue){if(oldValue!==newValue){this.render();this._attachListeners();}}
set data(routine){this.setAttribute('title',routine.name);this.setAttribute('count',routine.exercises?routine.exercises.length:0);this.setAttribute('id',routine.id);if(routine.lastPerformed){this.setAttribute('last-performed',routine.lastPerformed);}}
_formatLastPerformed(timestamp){const t=(key,vars)=>this.i18n?this.i18n.t(key,vars):'';if(!timestamp)return t('lbl_new')||'Nueva';const days=Math.floor((Date.now()-timestamp)/(1000*60*60*24));if(days===0)return t('lbl_today')||'Hoy';if(days===1)return t('lbl_yesterday')||'Ayer';return t('lbl_days_ago',{days:days})||`Hace ${days} días`;}
render(){const t=(key)=>this.i18n?this.i18n.t(key):'';const title=this.getAttribute('title')||t('fallback_title')||'Rutina Sin Nombre';const count=parseInt(this.getAttribute('count')||'0');const lastPerf=this.getAttribute('last-performed');const lastPerfText=lastPerf?` • ${this._formatLastPerformed(parseInt(lastPerf))}`:'';const exerciseWord=count===1?(t('lbl_exercise_count_single')||'ejercicio'):(t('lbl_exercise_count_plural')||'ejercicios');const iconEdit=`
        ...
        `;}
_attachListeners(){const mainArea=this.shadowRoot.getElementById('main-area');if(mainArea){mainArea.addEventListener('click',()=>{this.dispatchEvent(new CustomEvent('play',{detail:{id:this.getAttribute('id')},bubbles:true,composed:true}));});}
const btnEdit=this.shadowRoot.getElementById('btn-edit');if(btnEdit){btnEdit.addEventListener('click',(e)=>{e.stopPropagation();this.dispatchEvent(new CustomEvent('edit',{detail:{id:this.getAttribute('id')},bubbles:true,composed:true}));});}
const btnDelete=this.shadowRoot.getElementById('btn-delete');if(btnDelete){btnDelete.addEventListener('click',(e)=>{e.stopPropagation();this.dispatchEvent(new CustomEvent('delete',{detail:{id:this.getAttribute('id')},bubbles:true,composed:true}));});}}}
