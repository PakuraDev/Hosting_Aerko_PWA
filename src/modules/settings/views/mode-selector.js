import{ICONS}from'../../../core/theme/icons.js';import{i18nService}from'../../../core/i18n/i18n.service.js';import{SUPPORTED_LANGUAGES,MODES_INFO}from'../../../core/i18n/config.js';import'../../system/components/btn.js';export class SettingsModeSelector extends HTMLElement{constructor(){super();this.prefs=i18nService.getPreferences();this.selectedModeChar=this.prefs.mode;this.dict=null;const currentLangConfig=SUPPORTED_LANGUAGES.find(l=>l.id===this.prefs.lang);this.availableModes=currentLangConfig?currentLangConfig.type:'a';}
async connectedCallback(){this.dict=await i18nService.loadPage('system/config');this.render();this.addListeners();}
render(){if(!this.dict)return;const isAlreadyActive=this.selectedModeChar===this.prefs.mode;const currentInfo=MODES_INFO[this.selectedModeChar];const btnVariant=isAlreadyActive?'secondary':'primary';const btnLabelKey=isAlreadyActive?'btn_already_selected':'btn_save';this.innerHTML=`
            <style>
        ...
        `;}
addListeners(){this.querySelector('#btn-back').addEventListener('click',()=>window.history.back());this.querySelectorAll('.mode-row').forEach(row=>{row.addEventListener('click',()=>{this.selectedModeChar=row.dataset.char;this.render();this.addListeners();});});this.querySelector('#btn-save').addEventListener('click',async()=>{if(this.selectedModeChar!==this.prefs.mode){await i18nService.setMode(this.selectedModeChar);window.history.back();}});}}
