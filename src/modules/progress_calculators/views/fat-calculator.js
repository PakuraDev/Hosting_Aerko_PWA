import{router}from'../../../core/router/index.js';import{progressStore}from'../../progress_core/store/index.js';import{userService}from'../../user/services/user.service.js';import{i18nService}from'../../../core/i18n/i18n.service.js';import'../../system/components/btn.js';import'../../system/components/input-card.js';import'../../system/components/box.js';import'../../system/components/keypad-modal.js';import'../../system/components/navbar.js';import'../../system/components/unit-toggle.js';export class ProgressFatCalculator extends HTMLElement{constructor(){super();this.age=null;this.gender=null;this.folds={pectoral:'',abdominal:'',quadriceps_fold:'',suprailiac:'',triceps_fold:'',subscapular:'',midaxillary:''};this.usedFormula='';this.foldLabels={};this.foldLabels={pectoral:'Pectoral',abdominal:'Abdominal',quadriceps_fold:'Cuádriceps',suprailiac:'Suprailíaco',triceps_fold:'Tríceps',subscapular:'Subescapular',midaxillary:'Axilar_Medio'};}
async connectedCallback(){this.dict=await i18nService.loadPage('progress_calculators/fat-calculator');this.usedFormula=this.dict.t('calc_formula_none');this.foldLabels={pectoral:this.dict.t('fold_pectoral'),abdominal:this.dict.t('fold_abdominal'),quadriceps_fold:this.dict.t('fold_quadriceps'),suprailiac:this.dict.t('fold_suprailiac'),triceps_fold:this.dict.t('fold_triceps'),subscapular:this.dict.t('fold_subscapular'),midaxillary:this.dict.t('fold_midaxillary')};await this._loadUserData();this._loadFromDraft();this.render();this._attachListeners();this._updateResultsUI();}
async _loadUserData(){const profile=userService.getProfile();if(profile.age)this.age=profile.age;if(profile.gender)this.gender=profile.gender;}
_loadFromDraft(){const draft=progressStore.getDraft();Object.keys(this.folds).forEach(key=>{if(draft[key]&&draft[key].value!==""){this.folds[key]=draft[key].value;}});if(draft.fat&&draft.fat.value!==""){this.resultFat=parseFloat(draft.fat.value);}}
render(){const needsProfileData=!this.age||!this.gender;this.innerHTML=`
            <style>
            ...
            <div class="screen-container">
                ...
                <main class="content-scroll">
                    <div class="form-stack">
                        ...
                        ${needsProfileData ? `<div class="alert-msg">${this.dict.t('calc_alert_missing')}</div><div class="split-row"><app-input-card label="${this.dict.t('calc_lbl_age')}"><app-box id="box-age"clickable><span id="val-age"class="value-text ${this.age ? '' : 'placeholder-text'}">${this.age||'25'}</span></app-box></app-input-card><app-input-card label="${this.dict.t('calc_lbl_gender')}"><app-box id="box-gender"clickable><span id="val-gender"class="value-text ${this.gender ? '' : 'placeholder-text'}">${this.gender==='M'?this.dict.t('calc_gender_m'):(this.gender==='F'?this.dict.t('calc_gender_f'):this.dict.t('calc_gender_none'))}</span></app-box></app-input-card></div>` : ''}

                        ${Object.keys(this.folds).map(key => `<app-input-card label="${this.foldLabels[key]} (mm)"><app-box id="box-${key}"clickable><span id="val-${key}"class="value-text ${this.folds[key] === '' ? 'placeholder-text' : ''}">${this.folds[key]!==''?this.folds[key]:'0'}</span></app-box></app-input-card>`).join('')}
...
if(!customElements.get('progress-fat-calculator')){customElements.define('progress-fat-calculator',ProgressFatCalculator);}