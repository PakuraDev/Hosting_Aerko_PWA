...
class I18nService {
    ...
    async init() {
        try {
            ...
            // 1. ¿Hay preferencias guardadas (ya sea manuales o previas)? Las usamos.
            if (record && record.data && record.data.lang) {
                this._currentLang = record.data.lang;
                this._currentMode = record.data.mode || DEFAULT_MODE;
            } 
            // 2. ¿No hay nada guardado? (Es la primera vez que entra). Detectamos el navegador.
            else {
                ...
                // Guardamos esta primera detección en la BD para que no lo vuelva a calcular.
    ...
    async loadPage(pageName) {
        ...
        if (modePath !== basePath) {
            try {
                ...
                // Falla silenciosamente si no existe el modo para esa página
            }
        }
