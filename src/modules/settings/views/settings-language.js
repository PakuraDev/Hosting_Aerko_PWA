import{ICONS}from'../../../core/theme/icons.js';import{i18nService}from'../../../core/i18n/i18n.service.js';import{SUPPORTED_LANGUAGES}from'../../../core/i18n/config.js';import'../../system/components/btn.js';export class SettingsLanguage extends HTMLElement{constructor(){super();this.prefs=i18nService.getPreferences();this.selectedLang=this.prefs.lang;this.dict=null;}
            ...
            <div class="settings-screen">
                ...
                <main class="content-scroll">
                    ${SUPPORTED_LANGUAGES.map(lang => {
                        ...
                        return `<div class="lang-card ${isActive ? 'active' : ''}"data-id="${lang.id}"><div class="lang-header">${this.dict.t(lang.nameKey)}</div><div class="lang-modes"><div class="mode-badge">${this.dict.t('mode_default_label')}</div>${hasZen?`<div class="mode-badge">${this.dict.t('lang_badge_zen')}</div>`:''}
${hasTsu?`<div class="mode-badge">${this.dict.t('lang_badge_tsu')}</div>`:''}</div></div>`;
                    }).join('')}
                </main>
        ...
        `;}
addListeners(){this.querySelector('#btn-back').addEventListener('click',()=>window.history.back());this.querySelectorAll('.lang-card').forEach(card=>{card.addEventListener('click',()=>{this.selectedLang=card.dataset.id;this.render();this.addListeners();});});this.querySelector('#btn-save').addEventListener('click',async()=>{await i18nService.setLanguage(this.selectedLang);window.history.back();});}}
