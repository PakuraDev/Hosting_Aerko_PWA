import{ICONS}from'../../../core/theme/icons.js';import{i18nService}from'../../../core/i18n/i18n.service.js';export class AppMealSection extends HTMLElement{static get observedAttributes(){return['title'];}
constructor(){super();this.attachShadow({mode:'open'});}
async connectedCallback(){this.dict=await i18nService.loadPage('nutrition_smart/meal-section');this.render();this._setupListeners();}
attributeChangedCallback(name,oldValue,newValue){if(oldValue!==newValue)this.render();}
render(){if(!this.dict)return;const title=this.getAttribute('title')||this.dict.t('fallback_title');this.shadowRoot.innerHTML=`
        <style>
    ...
    <button class="sidebar-btn sidebar-btn--delete" id="btn-delete-section" aria-label="${this.dict.t('aria_delete')}">
        ...
        `;}
_setupListeners(){this.shadowRoot.getElementById('btn-edit-section').addEventListener('click',(e)=>{e.stopPropagation();this.dispatchEvent(new CustomEvent('edit-section',{bubbles:true,composed:true}));});this.shadowRoot.getElementById('btn-delete-section').addEventListener('click',(e)=>{e.stopPropagation();this.dispatchEvent(new CustomEvent('delete-section',{bubbles:true,composed:true}));});}}
