...
export class NutritionWizardForm extends HTMLElement {
    ...
    addListeners() {
        ...
        this.querySelector('#box-height').addEventListener('click', async () => {
            ...
            const result = await modal.open(this.dict.t('label_height'), this.formData.height, 'dynamic', this.formData.unitHeight, 'CM, IN', onRealTimeUnitChange);
            
            if (result !== null) {
                const valor = parseFloat(result.value);
                // Validación: CM (50-250) o Pulgadas (20-100)
                const esValido = (result.unit === 'CM') ? (valor >= 50 && valor <= 250) : (valor >= 20 && valor <= 100);
                if (esValido) {
        ...
        this.querySelector('#box-age').addEventListener('click', async () => {
            ...
            if (result !== null) {
                ...
                // Validación: Edad entre 1 y 110 años
                if (valor >= 1 && valor <= 110) {
        ...
        this.querySelector('#btn-next').addEventListener('click', () => {
            const baseHeight = this.formData.height !== '--' ? unitService.toBase(this.formData.height, this.formData.unitHeight) : '';
            wizardStore.update({
